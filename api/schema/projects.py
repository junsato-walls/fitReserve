...
class ProjectSearchQuery(PaginationQuery):
    """プロジェクト一覧の検索クエリ（管理用）"""

    include_deleted: bool = Field(False, description="削除済みを含めるか")


class ProjectPublicQuery(BaseModel):
    """予約受付用プロジェクトの取得クエリ（顧客向け）"""

    company_slug: str = Field(..., description="会社スラッグ（予約URLの先頭）")
    store_id: Optional[int] = Field(
        None, description="店舗ID（指定時は対象店舗か検証する）"
    )
