...
class SchoolSearchQuery(PaginationQuery):
    """学校一覧の検索クエリ（管理用）"""

    school_divisions_id: Optional[int] = Field(None, description="学校区分IDで絞る")
    include_deleted: bool = Field(False, description="削除済みを含めるか")


class SchoolPublicQuery(BaseModel):
    """学校一覧の検索クエリ（顧客向け）"""

    store_id: Optional[int] = Field(
        None, description="店舗ID（その店舗が取り扱う学校のみ）"
    )
    project_id: Optional[int] = Field(
        None, description="プロジェクトID（本日受付中の学校区分のみ）"
    )
