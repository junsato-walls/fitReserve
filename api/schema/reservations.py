...
class ReservationSearchQuery(PaginationQuery):
    """予約一覧の検索クエリ（社内向け）"""

    store_id: Optional[int] = Field(None, description="店舗IDで絞る")
    school_id: Optional[int] = Field(None, description="学校IDで絞る")
    status: Optional[ReservationStatus] = Field(None, description="ステータスで絞る")
    date_from: Optional[date] = Field(None, description="予約日の下限")
    date_to: Optional[date] = Field(None, description="予約日の上限")
