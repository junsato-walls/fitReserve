...
export const Banner = forwardRef<HTMLDivElement, BannerProps>(({
    ...
    return (
        ...
        >
            {/* メインコンテンツ */}
            <div className={`flex items-center mx-auto ${contentClassName}`}>
                <div className={`flex items-center text-sm font-normal ${variantStyles.text}`}>
                    {/* アイコン */}
                    {(icon !== null) && (
                        <span className={`
            ...
            {/* アクション＆クローズボタン */}
            <div className={`flex items-center space-x-2 ${actionsClassName}`}>
                {/* カスタムアクション */}
                {actions.map((action) => {
                    const baseActionClasses = "text-sm font-medium rounded-lg px-3 py-1.5";

                    const getActionClasses = () => {
                        switch (action.variant) {
                            case 'primary':
                                return `${baseActionClasses} bg-blue-600 text-white hover:bg-blue-700 dark:bg-blue-500 dark:hover:bg-blue-600`;
                            case 'secondary':
                                return `${baseActionClasses} border border-gray-300 text-gray-700 hover:bg-gray-50 dark:border-gray-600 dark:text-gray-300 dark:hover:bg-gray-700`;
                            case 'link':
                                return "text-sm font-medium hover:underline";
                            default:
                                return `${baseActionClasses} ${variantStyles.text} hover:bg-opacity-20`;
                        }
                    };
                ...
                {dismissible && (
                    <button
                        type="button"
                        className={`
                            shrink-0 inline-flex justify-center w-7 h-7 items-center
