...
export interface DatepickerProps extends Omit<
    ...
    "onChange" | "value" | "size" | "defaultValue" | "onFocus" | "onBlur"
> {
    // 値の設定
...
export const Datepicker = forwardRef<HTMLInputElement, DatepickerProps>(
    (
        ...
        const defaultIcon = (
            <svg
                className={cn(sizeClasses[size].iconSize, "text-gray-500 dark:text-gray-400")}
                aria-hidden="true"
        ...
        const isDateDisabled = (date: Date): boolean => {
            ...
            const min = minDate
                ? toDayStart(typeof minDate === "string" ? new Date(minDate) : minDate)
                : null
            const max = maxDate
                ? toDayStart(typeof maxDate === "string" ? new Date(maxDate) : maxDate)
                : null
            ...
            if (
                ...
                !availableDates.some((availableDate) => {
                    const available =
                        typeof availableDate === "string" ? new Date(availableDate) : availableDate
            ...
            if (
                disabledDates.some((disabledDate) => {
                    const disabled =
                        typeof disabledDate === "string" ? new Date(disabledDate) : disabledDate
        ...
        return (
            <div ref={containerRef} className={cn("relative", inline ? "" : "max-w-sm")}>
                {label && (
                    <label
                        htmlFor={baseId}
                        className={cn(
                ...
                {!inline && (
                    <input
                        ...
                        aria-activedescendant={
                            isOpen && activeDate ? getDayId(activeDate) : undefined
                        }
                        aria-invalid={error ? true : undefined}
                        aria-describedby={error ? errorId : undefined}
                        className={cn(
                            "bg-gray-50 dark:bg-gray-800 border border-gray-300 dark:border-gray-600 text-gray-900 dark:text-gray-50 rounded-lg focus:ring-blue-500 dark:focus:ring-blue-800 focus:border-blue-500 dark:focus:border-blue-500 block w-full dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500 disabled:opacity-50 disabled:cursor-not-allowed",
                            sizeClasses[size].input,
                            error
                                ? "border-red-500 dark:border-red-500 focus:border-red-500 dark:focus:border-red-500 focus:ring-red-500 dark:focus:ring-red-800"
                                : "",
                        )}
                        {...props}
                    />
                ...
                {isCalendarOpen && (
                    <div
                        ...
                        className={cn(
                            "bg-white border border-gray-200 rounded-lg dark:bg-gray-700 dark:border-gray-600 p-3",
                            inline ? "w-full" : "absolute z-50 mt-1 shadow-lg",
                        )}
                    >
                        {/* ヘッダー */}
                                            ...
                                            .map((date, index) => {
                                                ...
                                                return (
                                                    <button
                                                        ...
                                                        disabled={isDisabled}
                                                        className={cn(
                                                            "flex h-11 w-full min-w-11 items-center justify-center rounded-lg text-base hover:bg-blue-100 dark:hover:bg-blue-900",
                                                            isCurrentMonth
                                                                ? "text-gray-900 dark:text-white"
                                                                : "text-gray-400 dark:text-gray-500",
                                                            isSelected
                                                                ? "bg-blue-600 dark:bg-blue-600 text-white hover:bg-blue-700 dark:hover:bg-blue-600"
                                                                : "",
                                                            isToday && !isSelected
                                                                ? "bg-blue-100 text-blue-600 dark:bg-blue-900 dark:text-blue-300"
                                                                : "",
                                                            isActive && !isSelected
                                                                ? "ring-2 ring-blue-500 dark:ring-blue-800"
                                                                : "",
                                                            isDisabled
