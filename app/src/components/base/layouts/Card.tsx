...
const Card = forwardRef<HTMLElement, CardProps>(({
    ...
    const getVariantClasses = () => {
        const variants = {
            ...
            elevated: 'bg-white shadow-md dark:bg-gray-800 border border-gray-100 dark:border-gray-700',
        ...
        return isClickable ? `${variants[variant]} ${hoverVariants[variant]}` : variants[variant];
    };
    ...
    const contentElement = (
        <div className={`${getSizeClasses()} ${imagePosition === 'left' || imagePosition === 'right' ? 'flex-1' : ''}`}>
            {(title || headerActions) && (
                <div className="mb-3 flex items-center justify-between gap-3">
                    <div className={`min-w-0 ${titleAlign === 'center' ? 'flex-1 text-center' : ''}`}>
                        {title && (
                            <h5 className={`flex items-center gap-2 tracking-tight ${titleAlign === 'center' ? 'justify-center' : ''} ${titleSizeClasses[titleSize]}`}>
                                {titleIcon}
                                {title}
                            </h5>
                        )}
    ...
    const cardContent = (
        <>
            ...
            {(imagePosition === 'left' || imagePosition === 'right') && (
                <div className={`flex ${imagePosition === 'left' ? 'flex-row' : 'flex-row-reverse'}`}>
                    {imageElement}
                    {contentElement}
                </div>
            )}
            {(imagePosition === 'top' || imagePosition === 'bottom' || !image) &&
                imagePosition !== 'left' && imagePosition !== 'right' && contentElement}
