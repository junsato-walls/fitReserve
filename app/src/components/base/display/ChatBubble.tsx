...
export const ChatBubble = forwardRef<HTMLDivElement, ChatBubbleProps>(
    (
            ...
            switch (attachment.type) {
                ...
                case "file":
                    return (
                        <div
                            key={attachment.id}
                            className="mt-2 p-3 bg-white/10 dark:bg-gray-800/10 rounded-lg flex items-center space-x-3"
                        >
                ...
                case "link":
                    return (
                        <div
                            key={attachment.id}
                            className="mt-2 p-3 bg-white/10 dark:bg-gray-800/10 rounded-lg"
                        >
                            <a
                                href={attachment.url}
        ...
        const bubbleClasses = `
    flex flex-col w-full leading-1.5 p-4 border-gray-200 dark:border-gray-700
    ${getMaxWidthClasses()}
    ${getVariantClasses()}
    ${getRoundedClasses()}
  `
        ...
        return (
            <div ref={ref} id={id} className={cn("flex items-start gap-2.5", containerClasses)}>
                {/* アバター */}
                {showAvatar && position === "left" && (
                    <button onClick={onAvatarClick} className="shrink-0" disabled={!onAvatarClick}>
                        {user.avatar ? (
                            <Image
                                src={user.avatar}
                ...
                {/* メッセージバブル */}
                <div className={bubbleClasses} onClick={onMessageClick}>
                    {/* ヘッダー（名前と時刻） */}
