...
export const Timetable = ({
    ...
    const handlePointerUp = (event: React.PointerEvent) => {
        ...
        onMove(
            current.itemId,
            buildRange(current.columnIndex, current.startSlot, current.slots)
        );
        setAnnouncement(
    ...
    const handleItemKeyDown = (
        ...
        if (event.key === " ") {
            ...
            setAnnouncement(
                "移動を開始しました。矢印キーで動かし、Enterで確定、Escで取り消します"
            );
            return;
        }
    ...
    const commitKeyboardMove = () => {
        ...
        onMove?.(
            preview.itemId,
            buildRange(preview.columnIndex, preview.startSlot, preview.slots)
        );
        setAnnouncement(
        ...
        <div className="relative">
                    ...
                    <div className="flex">
                        ...
                        >
                            {columns.map((column, columnIndex) => (
                                ...
                                >
                                    ...
                                    {Array.from({ length: totalSlots }, (_, slot) => (
                                        <div
                                            key={slot}
                                            className={`absolute inset-x-0 border-t pointer-events-none ${(slot * slotMinutes) % 60 === 0
                                    ...
                                    {preview?.mode === "create" &&
                                        preview.columnIndex === columnIndex && (
                                            <div
                                                className={`absolute inset-x-1 z-10 rounded-md border-2 border-dashed pointer-events-none flex items-center justify-center text-xs ${preview.invalid
                            ...
                            {placed.map((entry) => {
                                const moving =
                                    preview?.mode === "move" &&
                                    preview.itemId === entry.item.id;
                                const columnIndex = moving
                                    ? preview.columnIndex
                                    : entry.columnIndex;
                                ...
                                return (
                                    <button
                                        key={entry.item.id}
                                        type="button"
                                        className={`absolute z-10 overflow-hidden rounded-md border px-2 py-1 text-left text-xs transition-shadow touch-none ${TONE_CLASSES[entry.item.tone ?? "info"]
                                            } ${moving
                                                ? preview.invalid
                                        ...
                                        style={{
                                            ...
                                            left: `calc(${columnIndex * columnWidth + entry.lane * laneWidth
                                                }% + 2px)`,
                                        ...
                                        aria-label={`${entry.item.title} ${toTimeLabel(
                                            start
                                        )}から${toTimeLabel(end)} ${columns[columnIndex].label
                                            }${entry.item.locked ? "（移動不可）" : ""}`}
                                        onPointerDown={(event) =>
                                            handleItemPointerDown(event, entry)
                                        }
            ...
            {(onMove || onCreate) && (
                <p className="mt-3 text-xs text-gray-500 dark:text-gray-400">
                    {onMove && "予定はドラッグで移動できます（キーボードはSpaceで掴み、矢印キーで移動、Enterで確定）。"}
