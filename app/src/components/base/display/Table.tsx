...
import { BADGE_BASE_CLASS, BADGE_TONE_CLASSES } from "./Badge"
...
export interface TableColumn<T = unknown> {
    ...
    badgeTone?: (value: unknown, row: T) => Tone
...
/** 真偽値表示の色 */
const BOOLEAN_TONE = {
    true: "text-green-600 dark:text-green-400 font-medium",
    false: "text-gray-500 dark:text-gray-400",
}

const TableComponent = <T = unknown,>(props: TableProps<T>, ref: React.Ref<HTMLDivElement>) => {
    const {
        data,
        columns,
        selectable = false,
    ...
    const getCellValue = (row: T, column: TableColumn<T>): ReactNode => {
        ...
        if (type === "badge") {
            ...
            return <span className={cn(BADGE_BASE_CLASS, BADGE_TONE_CLASSES[tone])}>{text}</span>
        }
    ...
    const isPartiallySelected =
        data.some((row, index) => selectedRows.has(getRowId(row, index))) && !isAllSelected
  ...
  `
    ...
    const getRowBgClasses = (index: number) =>
        striped && index % 2 === 1 ? "bg-gray-50 dark:bg-gray-900" : "bg-white dark:bg-gray-800"
    ...
    return (
        <div
            ref={ref}
            className={cn("relative overflow-x-auto", bordered ? "shadow-md sm:rounded-lg" : "")}
        >
                            ...
                            <th scope="col" className={compact ? "p-2" : "p-4"}>
                                <div className="flex items-center">
                                    <input
                                        ...
                                        className="w-4 h-4 text-blue-600 dark:text-blue-400 bg-gray-100 border-gray-300 rounded-sm focus:ring-blue-500 dark:focus:ring-blue-600 dark:ring-offset-gray-800 dark:focus:ring-offset-gray-800 focus:ring-2 dark:bg-gray-700 dark:border-gray-600"
                    ...
                    {loading ? (
                        <tr>
                            <td
                                ...
                                className={
                                    compact
                                        ? "px-3 py-4 text-center text-gray-500 dark:text-gray-400"
                                        : "px-6 py-8 text-center text-gray-500 dark:text-gray-400"
                                }
                            >
                                ...
                                <tr key={rowId} className={getRowClasses(index)}>
                                    ...
                                    {selectable && (
                                        <td className={getCheckboxCellClasses()}>
                                            <div className="flex items-center">
                                                ...
                                                />
                                                <label
                                                    htmlFor={`checkbox-table-search-${rowId}`}
                                                    className="sr-only"
                                                >
                                                    checkbox
                                                </label>
                                    ...
                                    {columns.map((column, colIndex) => {
                                        ...
                                        return isFirstColumn ? (
                                            ...
                                            <td key={column.id} className={getCellClasses(column)}>
                                                {cellValue}
                                            </td>
                                        )
                                    })}
                                            ...
                                            {actions.map((action) =>
                                                action.href ? (
                                                    ...
                                                    <button
                                                        ...
                                                        className={cn(
                                                            "font-medium hover:underline disabled:opacity-40 disabled:cursor-not-allowed disabled:no-underline",
                                                            action.destructive
                                                                ? "text-red-600 dark:text-red-500"
                                                                : "text-blue-600 dark:text-blue-500",
                                                            action.id === actions[0].id
...
export const Table = forwardRef(TableComponent) as TableComponentType
